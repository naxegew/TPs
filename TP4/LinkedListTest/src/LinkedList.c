 ...
 */
static int addNode(LinkedList *this, int nodeIndex, void *pElement) {
	int returnAux = -1, anterior = nodeIndex - 1;
	...
	if (this != NULL && nodeIndex >= 0 && newNode != NULL) {
		...
		if (nodeIndex == 0) {
			...
			antNode = getNode(this, anterior);
			newNode->pNextNode = antNode->pNextNode;
			antNode->pNextNode = newNode;
			this->size++;
			returnAux = 0;
		}
	}
...
/** \brief Devuelve el elemento en el nodo indicado por el index.
 *
 * \param this LinkedList* Puntero a la lista
...
int ll_remove(LinkedList *this, int index) {
	...
	if (node != NULL && index >= 0 && index <= this->size) {
		if (index == 0) {
			...
			nodeAnt->pNextNode = node->pNextNode;
			this->size--;
		}
		returnAux = 0;
...
int ll_containsAll(LinkedList *this, LinkedList *this2) {
	...
	if (this != NULL && this2 != NULL) {
		...
		node2 = this2->pFirstNode;
		returnAux = 1;
		for (i = 0; i < this->size; i++) {
			for (j = 0; j < this2->size; j++) {
				if (node->pElement != node2->pElement) {
					returnAux = 0;
					break;
				}
			}
...
LinkedList* ll_subList(LinkedList *this, int from, int to) {
	...
	int i;
	if (this != NULL && from >= 0 && from <= this->size && to > from
			&& to <= this->size) {
